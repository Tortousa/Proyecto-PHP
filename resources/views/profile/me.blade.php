...
@section('header')
    <div class="flex justify-between items-center">
            ...
            <a href="{{ route('profile.edit') }}"
               class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-bold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 transition">
                {{ __('Edit Profile') }}
            </a>
        </div>
@endsection
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
            ...
            <div class="bg-white shadow sm:rounded-lg p-6 flex items-center gap-6">
                ...
                <div>
                    ...
                    <span class="mt-2 inline-block px-3 py-1 text-xs font-semibold rounded-full
                        {{ $user->hasRole('admin') ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700' }}">
            ...
            <div class="bg-white shadow sm:rounded-lg p-6">
                <div class="flex justify-between items-center mb-6">
                    <h3 class="text-lg font-bold text-gray-800">{{ __('My Cars') }}</h3>
                    <a href="{{ route('cars.create') }}"
                       class="inline-flex items-center px-3 py-2 bg-indigo-600 text-white text-xs font-bold uppercase rounded-md hover:bg-indigo-700 transition">
                        + {{ __('New Car') }}
                    </a>
                </div>

                @if($user->cars->isEmpty())
                    <p class="text-gray-500 text-center py-8">{{ __("You haven't listed any cars yet.") }}</p>
                @else
            ...
            {{-- Mis favoritos --}}
            <div class="bg-white shadow sm:rounded-lg p-6">
                <h3 class="text-lg font-bold text-gray-800 mb-6">{{ __('My Favourites') }}</h3>

                @if($user->favouriteCars->isEmpty())
                    <p class="text-gray-500 text-center py-8">{{ __("You haven't saved any favourites yet.") }}</p>
                @else
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
